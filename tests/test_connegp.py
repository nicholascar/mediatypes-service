...
def test_mediatypes_register_reg_ttl_http():
    ...
    assert r.headers['Content-Type'].startswith('text/turtle'), \
        'Media Types Register does not have a header of Content-Type text/turtle, instead {}'\
            .format(r.headers['Content-Type'])
    assert 'rdfs:label "Register of Media Types"^^xsd:string ;' in r.text, \
...
def test_mediatypes_register_reg_ttl_qsa():
    ...
    assert r.headers['Content-Profile'] == '<http://purl.org/linked-data/registry>', \
        ...
        'instead {}'.format(r.headers['Content-Profile'])
    assert r.headers['Content-Type'].startswith('text/turtle'), \
        'Media Types Register does not have a header of Content-Type, instead {}'.format(r.headers['Content-Type'])
    assert 'rdfs:label "Register of Media Types"^^xsd:string ;' in r.text, \
        'Media Types Register does not include rdfs:label "Register of Media Types"^^xsd:string ;'
...
def test_mediatypes_register_alternates_html_http():
    ...
    assert r.headers['Content-Type'].startswith('text/html'), \
        'Media Types Register does not have a header of Content-Type: text/html, instead {}'\
            .format(r.headers['Content-Type'])
    assert '<h1>Alternates View</h1>' in r.text, \
...
def test_mediatypes_register_alternates_ttl_http():
    ...
    assert r.headers['Content-Type'].startswith('text/turtle'), \
        'Media Types Register does not have a header of Content-Type: text/turtle, instead {}'\
            .format(r.headers['Content-Type'])
    assert 'rdfs:label "Alternates"^^xsd:string ;' in r.text, \
...
def test_link_header_self_mediatype():
    r = requests.get(
        ...
        params={
            'uri': 'https://w3id.org/mediatype/text/n3'
        }
    )
    ...
    assert \
        uri == expected_header['uri'] and type == expected_header['type'] and profile == expected_header['profile'], \
        'The \'self\' URI in the Link header was {} {} {}, should have been {} {} {}.'\
            .format(
            uri, type, profile, expected_header['uri'], expected_header['type'], expected_header['profile']
        )
